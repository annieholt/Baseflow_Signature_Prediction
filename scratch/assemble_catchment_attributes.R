...
new_attrib = nwi_c %>% 
  ...
  select(gauge_id, giw_frac, fresh_no_giw, geol_av_age_ma, geol_major_age_ma) %>% 
  mutate(giw_frac = ifelse(giw_frac <0, 0, giw_frac)) 


# also including major lithology classifications, separately for now
...
#### Correlation analysis (for plotting and final attribute selections) ####
...
rf_camels = camels_attribs %>% 
  ...
  left_join(sigs_c_2 %>% select(gauge_id, RecessionParameters_b), by = 'gauge_id') %>% 
  left_join(new_attrib, by = "gauge_id") %>% 
  left_join(new_attrib_lith, by = "gauge_id") %>% 
  select(-fresh_no_giw, -geol_major_age_ma) %>%
...
var_importance_2 = importance(rf_model_2, type = 1,scale = TRUE)

var_importance_2_df = as.data.frame(var_importance_2)
